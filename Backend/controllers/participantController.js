...
async function resetParticipantAccepted(req, res) {
    try {
        ...
        res.json({message: " totes les participant accepter sont reinitialiser", participants: actualise})
    } catch (error) {
        console.error('erreur de la recuperation des participants', error);
        res.status(500).send('erreur serveur');
    }
...
module.exports = { uploadFiles, getParticipants, upload, downloadFile, deleteParticipant, acceptedParticipant, resetParticipantAccepted};